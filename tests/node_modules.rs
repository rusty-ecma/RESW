...
fn jquery() {
    ensure_libraries();
    const PATH: &str = "./node_modules/jquery/dist/jquery.js";
    let js = ::std::fs::read_to_string(PATH).expect("failed to read js file");
    if let Err(e) = common::round_trip_validate(&js, false, "jquery") {
        let mut msg = format!("{e:?}");
        if let Error::Ressa(inner) = e {
            if let Some(pos) = inner.position() {
                msg.push_str(&format!(" {}:{}:{}", PATH, pos.line, pos.column));
            }
        }
        panic!("{}", msg);
    };
